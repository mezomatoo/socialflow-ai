#!/usr/bin/env python3
"""Reviewed additive OAuth migration for an existing inbox-enabled SQLite DB.
...
if len(sys.argv) != 2:
    sys.exit('Kullanım: python3 scripts/migrate-oauth.py prisma/dev.db')
path = pathlib.Path(sys.argv[1]).resolve()
if not path.is_file():
    sys.exit('Mevcut veritabanı gerekli.')
name = '202609150003_oauth_binding'
...
for migration, digest, finished, rolled_back in rows:
    ...
    if finished is not None and rolled_back is None:
        ...
        if migration == name:
            print('OAuth migration zaten uygulanmış; değişiklik yapılmadı.')
            sys.exit(0)
if not any(r[0] == '202609150002_inbox_foundation' and r[2] is not None and r[3] is None for r in rows):
    sys.exit('Önce Gelen Kutusu migration uygulanmalı.')
columns = {r[1] for r in c.execute('PRAGMA table_info("OAuthState")')}
if columns & {'socialAccountId', 'redirectUri', 'accountUpdatedAt'}:
    sys.exit('Kayıtsız OAuth şema değişikliği var; işlem durduruldu.')
...
backup = backup_dir / (path.stem + '-oauth-' + datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S%f') + '.db')
...
try:
    ...
    c.close()
print(f'OAuth migration tamamlandı; {len(counts)} tablonun kayıt sayısı korundu. Yedek: {backup.name}')
