#!/usr/bin/env python3
"""Reviewed additive CRM migration for an existing inbox-enabled SQLite DB.
...
if len(sys.argv) != 2:
    sys.exit('Kullanım: python3 scripts/migrate-crm.py prisma/dev.db')
path = pathlib.Path(sys.argv[1]).resolve()
if not path.is_file():
    sys.exit('Mevcut veritabanı gerekli.')
name = '202609150006_crm_leads'
...
for migration, digest, finished, rolled_back in rows:
    ...
    if finished is not None and rolled_back is None:
        ...
        if migration == name:
            print('CRM migration zaten uygulanmış; değişiklik yapılmadı.')
            sys.exit(0)
if not any(r[0] == '202609150005_product_catalog' and r[2] is not None and r[3] is None for r in rows):
...
if existing_tables & {'Contact', 'Lead', 'LeadActivity', 'ContactConversationLink'}:
    sys.exit('Kayıtsız CRM şema değişikliği var; işlem durduruldu.')
...
backup = backup_dir / (path.stem + '-crm-' + datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S%f') + '.db')
...
try:
    ...
    c.close()
print(f'CRM migration tamamlandı; {len(counts)} tablonun kayıt sayısı korundu. Yedek: {backup.name}')
