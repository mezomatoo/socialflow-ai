#!/usr/bin/env python3
"""Reviewed additive Katalog migration for an existing inbox-enabled SQLite DB.
...
if len(sys.argv) != 2:
    sys.exit('Kullanım: python3 scripts/migrate-catalog.py prisma/dev.db')
path = pathlib.Path(sys.argv[1]).resolve()
if not path.is_file():
    sys.exit('Mevcut veritabanı gerekli.')
name = '202609150005_product_catalog'
...
for migration, digest, finished, rolled_back in rows:
    ...
    if finished is not None and rolled_back is None:
        ...
        if migration == name:
            print('Katalog migration zaten uygulanmış; değişiklik yapılmadı.')
            sys.exit(0)
if not any(r[0] == '202609150004_advertising_foundation' and r[2] is not None and r[3] is None for r in rows):
    sys.exit('Önce Reklam migration uygulanmalı.')
existing_tables = {r[0] for r in c.execute("SELECT name FROM sqlite_master WHERE type='table'")}
if existing_tables & {'Product', 'ProductVariant', 'InventoryMovement'}:
    sys.exit('Kayıtsız katalog şema değişikliği var; işlem durduruldu.')
...
backup = backup_dir / (path.stem + '-catalog-' + datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S%f') + '.db')
...
try:
    ...
    c.close()
print(f'Katalog migration tamamlandı; {len(counts)} tablonun kayıt sayısı korundu. Yedek: {backup.name}')
