...
it('katalog ve CRM ekranları, API rotaları ve servisleri kaldırıldı', () => {
  for (const path of ['src/app/(app)/app/katalog', 'src/app/(app)/app/musteriler', 'src/app/api/catalog', 'src/app/api/crm', 'src/lib/catalog', 'src/lib/crm', 'src/lib/business']) {
...
it('Gelen Kutusu artık CRM sorgusu/linki taşımaz; diğer ana menüler korunur', () => {
  for (const file of ['src/lib/inbox/service.ts', 'src/lib/inbox/contracts.ts', 'src/app/(app)/app/gelen-kutusu/InboxView.tsx']) {
    assert.doesNotMatch(readFileSync(file, 'utf8'), /crmContactId|contactConversationLink|\/app\/musteriler/);
  }
  for (const href of ['/app/gelen-kutusu', '/app/reklamlar', '/sosyal-hesaplar', '/yeni-icerik', '/marka-kiti']) {
