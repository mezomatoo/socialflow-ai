/**
 * PHASE 4 — Özellik bayrakları (§137)
 * ---------------------------------------------------------------------------
 * Yeni AI/otomasyon yetenekleri varsayılan olarak KAPALIdır (güvenlik). Yalnızca
 * inşa etmekte olduğumuz Brand Kit çekirdeği (brandKitAdvanced, brandLock)
 * varsayılan AÇIKtır. Bayraklar `FF_<SCREAMING_SNAKE>` ortam değişkeniyle
 * ("true"/"false") geçersiz kılınabilir. Workspace-bazlı kalıcı bayraklar, admin
 * AI kullanım ekranıyla (ileriki adım) AppSettings üzerinden yönetilecektir; bu
 * katman şimdilik env + varsayılan okur ve TEK doğruluk kaynağıdır.
 */

export const FEATURE_FLAGS = [
  'brandKitAdvanced',
  'brandLock',
  'aiImageGeneration',
  'aiImageEditing',
  'generativeExpand',
  'smartCreativeResize',
  'aiVideoRepurposing',
  'aiContentPlanner',
  'aiCampaignBuilder',
  'predictivePerformance',
  'trendIntelligence',
  'competitorIntelligence',
  'automationEngine',
  'semanticSearch',
  'unifiedInbox',
  'directMessages',
  'socialListening',
  'reputationMonitoring',
  'reviewManagement',
  'agencyManagement',
  'clientPortal',
  'whiteLabel',
  'publicApi',
  'outgoingWebhooks',
  'subscriptions',
  'enterpriseSSO',
  'paidMedia',
  'paidMediaWrite',
  'metaAds',
  'googleAds',
  'youtubeAds',
  'linkedinAds',
  'tiktokAds',
  'xAds',
  'pinterestAds',
  'snapchatAds',
  'catalogAds',
  'adCreativeAdaptation',
  'adPolicyPrecheck',
  'aiMediaPlanner',
  'budgetRecommendations',
  'creativeFatigue',
  'conversionHub',
  'attribution',
  'revenueAnalytics',
  'creatorManagement',
  'ugcLibrary',
  'affiliateManagement',
  'socialCommerce',
] as const;
export type FeatureFlag = (typeof FEATURE_FLAGS)[number];

/** Varsayılan durumlar: güvenlik için çoğu AI/otomasyon bayrağı KAPALI. */
export const FEATURE_FLAG_DEFAULTS: Record<FeatureFlag, boolean> = {
  brandKitAdvanced: true,
  brandLock: true,
  aiImageGeneration: false,
  aiImageEditing: false,
  generativeExpand: false,
  smartCreativeResize: false,
  aiVideoRepurposing: false,
  aiContentPlanner: false,
  aiCampaignBuilder: false,
  predictivePerformance: false,
  trendIntelligence: false,
  competitorIntelligence: false,
  automationEngine: false,
  semanticSearch: false,
  unifiedInbox: true,
  directMessages: false,
  socialListening: false,
  reputationMonitoring: false,
  reviewManagement: false,
  agencyManagement: false,
  clientPortal: false,
  whiteLabel: false,
  publicApi: false,
  outgoingWebhooks: false,
  subscriptions: false,
  enterpriseSSO: false,
  paidMedia: true,
  paidMediaWrite: false,
  metaAds: false,
  googleAds: false,
  youtubeAds: false,
  linkedinAds: false,
  tiktokAds: false,
  xAds: false,
  pinterestAds: false,
  snapchatAds: false,
  catalogAds: false,
  adCreativeAdaptation: false,
  adPolicyPrecheck: false,
  aiMediaPlanner: false,
  budgetRecommendations: false,
  creativeFatigue: false,
  conversionHub: false,
  attribution: false,
  revenueAnalytics: false,
  creatorManagement: false,
  ugcLibrary: false,
  affiliateManagement: false,
  socialCommerce: false,
};

export const FEATURE_FLAG_LABELS: Record<FeatureFlag, string> = {
  brandKitAdvanced: 'Gelişmiş Marka Kiti',
  brandLock: 'Marka Kilidi',
  aiImageGeneration: 'AI Görsel Üretme',
  aiImageEditing: 'AI Görsel Düzenleme',
  generativeExpand: 'Üretken Genişletme',
  smartCreativeResize: 'Akıllı Kreatif Yeniden Boyutlandırma',
  aiVideoRepurposing: 'AI Video Dönüştürme',
  aiContentPlanner: 'AI İçerik Planlayıcı',
  aiCampaignBuilder: 'AI Kampanya Oluşturucu',
  predictivePerformance: 'Öngörüsel Performans',
  trendIntelligence: 'Trend İstihbaratı',
  competitorIntelligence: 'Rakip İstihbaratı',
  automationEngine: 'Otomasyon Motoru',
  semanticSearch: 'Anlamsal Arama',
  unifiedInbox: 'Gelen Kutusu',
  directMessages: 'Direkt Mesajlar',
  socialListening: 'Sosyal Dinleme',
  reputationMonitoring: 'Marka İzleme',
  reviewManagement: 'İncelemeler',
  agencyManagement: 'Ajans Yönetimi',
  clientPortal: 'Müşteri Portalı',
  whiteLabel: 'Beyaz Etiket',
  publicApi: 'Dış API',
  outgoingWebhooks: 'Giden Webhooklar',
  subscriptions: 'Plan ve Kullanım',
  enterpriseSSO: 'Kurumsal Tek Oturum',
  paidMedia: 'Reklam Yönetimi',
  paidMediaWrite: 'Reklam Yazma İşlemleri',
  metaAds: 'Meta Reklamları',
  googleAds: 'Google Reklamları',
  youtubeAds: 'YouTube Reklamları',
  linkedinAds: 'LinkedIn Reklamları',
  tiktokAds: 'TikTok Reklamları',
  xAds: 'X Reklamları',
  pinterestAds: 'Pinterest Reklamları',
  snapchatAds: 'Snapchat Reklamları',
  catalogAds: 'Katalog Reklamları',
  adCreativeAdaptation: 'Reklam Kreatif Uyarlama',
  adPolicyPrecheck: 'Reklam Politika Ön Kontrolü',
  aiMediaPlanner: 'AI Medya Planlama',
  budgetRecommendations: 'Bütçe Önerileri',
  creativeFatigue: 'Kreatif Yorgunluğu',
  conversionHub: 'Dönüşüm Merkezi',
  attribution: 'Atıf Analizi',
  revenueAnalytics: 'Gelir Analizi',
  creatorManagement: 'İçerik Üreticisi Yönetimi',
  ugcLibrary: 'Kullanıcı İçeriği Kütüphanesi',
  affiliateManagement: 'Satış Ortaklığı',
  socialCommerce: 'Sosyal Ticaret',
};

function envKey(flag: FeatureFlag): string {
  return 'FF_' + flag.replace(/([a-z0-9])([A-Z])/g, '$1_$2').toUpperCase();
}

/** Bir özellik bayrağı etkin mi? (env override > varsayılan) */
export function isFeatureEnabled(flag: FeatureFlag): boolean {
  const raw = process.env[envKey(flag)];
  if (raw === undefined || raw === '') return FEATURE_FLAG_DEFAULTS[flag];
  return raw === 'true' || raw === '1';
}

/** Tüm bayrakların etkin durumunu döner (admin/ayar ekranları için). */
export function featureFlagState(): Record<FeatureFlag, boolean> {
  return FEATURE_FLAGS.reduce(
    (acc, f) => {
      acc[f] = isFeatureEnabled(f);
      return acc;
    },
    {} as Record<FeatureFlag, boolean>
  );
}
